<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Home</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-Zenh87qX5JnK2Jl0vWa8Ck2rdkQ2Bzep5IDxbcnCeuOxjzrPF/et3URy9Bv1WTRi" crossorigin="anonymous">
  </head>
  <body>
    <nav class="navbar navbar-expand-lg bg-primary">
      <div class="d-flex justify-content-left container-fluid">
        <ul class="navbar-nav" style="padding-left:100px;color:white">
          <li class="nav-item"><a class= "nav-link" href="/Home">Home</a>
          <li class="nav-item"><a class= "nav-link" href="/Listcar">MyCar</a>
        </ul>
        <ul class="navbar-nav ms-auto" style="margin-right: 100px;">
           <li class="nav-item mx-5">
              <a href='/login' class="btn btn-light" type="button">Login</a>
              <a href='/Register-page' class="btn btn-light" type="button">Logout</a>
           </li>
         </ul>
      </div>
    </nav>
    <div class="row mx-auto w-75 p-3 mt-2">
      <div class="col-6 text-left" style="margin-top: 150px">
        <h1 style="font-size:330%; font-family: Arial; font-weight: 600" alt="center">Selamat Datang Di <br>
        Show Room Rydho </h1>
        <p style="font-size:90%" style="color:DDDDDD;">Selamat datang di Website Showroom WAD anda dapat merental mobil di Showroom Rydho </p>
        <a class="btn btn-primary" href="Add_RYDHO.php" role="button" style= "width:100px; height:45px">MyCar</a>
        <div class="mb-3">
          <div class="d-flex left">
          <img src="/img/logo-ead.png" style="width:100px;height:30px;float:left;margin-top:20px"/> <p style= "padding-left: 20px;margin-top:20px" >Rydho_1202204122</p>       
          </div>
        </div>
      </div>
      <div class="col-6 text-center">
        <img src="/img/MOBIL UTAMA.jpg" style="width:400px;margin-top: 150px" alt="">
      </div>
    </div>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-OERcA2EqjJCMA+/3y+gxIOqMEjwtxJY7qPCqsdltbNJuaOe923+mo//f6V8Qbsw3" crossorigin="anonymous"></script>
  </body>
</html>